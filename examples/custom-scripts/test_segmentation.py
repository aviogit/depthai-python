...
# create new node for segmentation masking
segmentation_colors = pipeline.create(dai.node.Script)
segmentation_colors.inputs['nn_in'].setBlocking(False)
detection_nn.out.link(segmentation_colors.inputs['nn_in'])  # connect the nn outputs to the segmentation colors node
segmentation_colors.setScriptData("""
while True:
        ...
        #send outputs to the nnout xlinkout node
        node.io['segmentation_colors'].send(output_colors)
""")

xout_nn = pipeline.createXLinkOut()
xout_nn.setStreamName("nn")
xout_nn.input.setBlocking(False)
segmentation_colors.outputs['output_colors'].link(xout_nn.input)
...
while True:
    ...
    if in_nn is not None:
        ...
        if frame is not None:
            frame = show_deeplabv3p(output, frame)
