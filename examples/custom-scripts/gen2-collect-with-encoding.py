...
start_timestamp_str = datetime.now().strftime("%Y%m%d-%H%M%S.%f")
path = Path(args.path).resolve().absolute()
dest = path.parent / (path.name + '-' + start_timestamp_str)
...
def store_frames(in_q):
    ...
    while True:
        ...
        for stream_name, item in frames_dict.items():
            ext = 'png' if 'disparity' in stream_name else 'jpg'
            cv2.imwrite(str(frames_path / Path(f"{stream_name}.{ext}")), item)
...
def store_encodings(dev):
    ...
    fileLeft = open(f'{dest}-{timestamp_str}/left_{timestamp_str}.h264', 'wb')
    fileColor = open(f'{dest}-{timestamp_str}/color_{timestamp_str}.h265', 'wb')
    fileRight = open(f'{dest}-{timestamp_str}/right_{timestamp_str}.h264', 'wb')
    fileDisparity = open(f'{dest}-{timestamp_str}/disparity_{timestamp_str}.h264', 'wb')
...
with dai.Device(pipeline) as device:
    ...
    allqueues = device.getInputQueueNames()
    print(f'{allqueues = }')
